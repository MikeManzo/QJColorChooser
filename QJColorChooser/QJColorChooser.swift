...
@objc @IBDesignable open class QJColorChooser: NSView, QJColorGridViewDelegate, NSAccessibilityButton, CALayerDelegate {
    ...
    /// The most recently clicked-on QJColorChooser.
    /// This is necessary for color panel integration.
    static var lastColorButton: QJColorChooser?
    ...
    var lastSelectionType: QJColorGridViewSelectionType = .colorGridSelection
    ...
    func colorWasSelected(_ sender: QJColorGridView, color: NSColor?, selectionType: QJColorGridViewSelectionType) {
        popover.close()
        var color = color
    ...
    fileprivate func resetPopoverView() {
        popover.contentViewController!.view =
            QJColorGridView(frame: CGRect(origin: CGPoint(x: 0.0, y: 0.0), size: popoverRequiredFrameSize()),
                            belongsTo: self)
    }
    ...
    @objc fileprivate func colorFromPanel(_ sender: AnyObject?) {
        ...
        // Look at the last QJColorChooser to have focus and alter it accordingly.
        if let lastColorButton = QJColorChooser.lastColorButton {
    ...
    override open func mouseUp(with theEvent: NSEvent?) {
        ...
        QJColorChooser.lastColorButton = self
        colorPanel.setTarget(self)
        colorPanel.setAction(#selector(self.colorFromPanel(_:)))
        showColorPopover()
    }
